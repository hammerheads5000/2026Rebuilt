...
public class EasyCRTConfig {
    ...
    public EasyCRTConfig withCommonDriveGear(
            double commonRatio, int driveGearTeeth, int absoluteEncoder1PinionTeeth, int absoluteEncoder2PinionTeeth) {
    ...
    public static double ratioFromChain(int... teethChain) {
        ...
        for (String stage : stages) {
            int first = Integer.parseInt(stage.split(":")[0]);
            int second = Integer.parseInt(stage.split(":")[1]);
            ratio *= first / second;
        }
        return ratio;
    }
    ...
    public Optional<CrtGearPair> getRecommendedCrtGearPair() {
        ...
        CrtGearPair pair = findSmallestCrtGearPair(
